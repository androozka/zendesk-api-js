...
describe('Tags', () => {
  ...
  beforeAll(() => (tags = endpoint(instance, headers)));
  afterAll(() => (tags = null));
  ...
  test('Show Tags', () => {
    expect(tags.show('tickets', 123)).toEqual({
      method: 'GET',
      url: `https://${instance}.zendesk.com/api/v2/tickets/123/tags.json`,
      headers
    });

    expect(tags.show('organizations', 123)).toEqual({
      method: 'GET',
      url: `https://${instance}.zendesk.com/api/v2/organizations/123/tags.json`,
      headers
    });

    expect(tags.show('users', 123)).toEqual({
      method: 'GET',
      url: `https://${instance}.zendesk.com/api/v2/users/123/tags.json`,
      headers
    });
  });
  ...
  test('Autocomplete Tags', () => {
    expect(tags.autocomplete('tag_name')).toEqual({
      method: 'GET',
      url: `https://${instance}.zendesk.com/api/v2/autocomplete/tags.json?name=tag_name`,
      headers
    });
  });
});
